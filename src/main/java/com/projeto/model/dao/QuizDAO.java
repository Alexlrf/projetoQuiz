...
public class QuizDAO {
	...
	public List<PerguntaVO> consultaPerguntasQuiz(int idRetornado) throws ErroNaConsultaException {
		...
		String sql = "SELECT" + " prova_quiz.id_pergunta, pergunta.texto_pergunta " + "FROM" + " prova_quiz \r\n"
				+ "INNER JOIN" + " pergunta on prova_quiz.id_pergunta = pergunta.id_pergunta " + "WHERE"
				+ " prova_quiz.id_quiz = ?;";
	...
	public List<QuizVO> consultaTodosQuizes(UsuarioVO usuarioLogado) throws ErroNaConsultaException {
		List<QuizVO> listaDeQuiz = new ArrayList<>();
		QuizVO quizVO;
		String sql = "SELECT * FROM quiz WHERE id_usuario = ?";
		...
		try (Connection conn = Banco.getConnection(); PreparedStatement stmt = Banco.getPreparedStatement(conn, sql)) {
			...
			while (rs.next()) {
				quizVO = new QuizVO();
				quizVO.setIdQuiz(rs.getInt("id_quiz"));
				listaDeQuiz.add(quizVO);
			}

		} catch (SQLException e) {
			System.out.println("Erro ao registrar resultado!");
			throw new ErroNaConsultaException("Erro ao pesquisar lista de Quiz!");
	...
	public List<QuizSeletor> buscaResultadosQuiz(int idQuizBusca) throws ErroNaConsultaException {
		List<QuizSeletor> listaResultados = new ArrayList<>();
		QuizSeletor quizSeletor;
		String sql = "SELECT * FROM resultado_prova WHERE id_quiz = ?";
		
		try (Connection conn = Banco.getConnection(); 
				PreparedStatement stmt = Banco.getPreparedStatement(conn, sql)) {

			stmt.setInt(1, idQuizBusca);
			ResultSet rs = stmt.executeQuery();

			while (rs.next()) {
				quizSeletor = new QuizSeletor();
				quizSeletor.setAcertos(rs.getInt("num_acertos"));
				int idAluno = rs.getInt("id_usuario");
				quizSeletor.setNomeAluno(buscaNomeAluno(idAluno));
				listaResultados.add(quizSeletor);
			}
	...
	private String buscaNomeAluno(int idAluno) throws ErroNaConsultaException {
		String nomeAluno = "";
		...
		try (Connection conn = Banco.getConnection(); 
			...
			System.out.println("Erro ao registrar resultado!");
			throw new ErroNaConsultaException("Erro ao pesquisar nome do aluno!");
